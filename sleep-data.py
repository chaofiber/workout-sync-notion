...
def format_date_for_name(sleep_date):
    return (
        datetime.strptime(sleep_date, "%Y-%m-%d").strftime("%d.%m.%Y") if sleep_date else "Unknown"
    )


def sleep_data_exists(client, database_id, sleep_date):
    query = client.databases.query(
        database_id=database_id, filter={"property": "Long Date", "date": {"equals": sleep_date}}
    )
...
def create_sleep_data(client, database_id, sleep_data, skip_zero_sleep=True):
    ...
    client.pages.create(
        parent={"database_id": database_id}, properties=properties, icon={"emoji": "😴"}
    )
